...
interface StripEmptyParamsOptions {
  /** Which tools to apply to: "mcp" (default) or "all". */
  scope?: "mcp" | "all";
  /** Log stripped keys to stderr for debugging. */
  verbose?: boolean;
}

const plugin: Plugin<StripEmptyParamsOptions> = async (_input, options) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      for (const key of Object.keys(args)) {
        ...
        if ((args as Record<string, unknown>)[key] === "") {
          if (verbose) {
